...
function buildKochState(start, end, maxDepth) {
  return {
    points: [start],
    stack: [
      {
...
export function mountKochBackground(options = {}) {
  ...
  const state = buildKochState(start, end, maxDepth);
...
export function mountKochBackgroundWhenReady(options = {}) {
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", () => mountKochBackground(options), {
      once: true,
    });
    return;
  }
