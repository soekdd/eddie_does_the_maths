...
<v-card class="pa-4" rounded="xl">
	<h3 class="text-h6 mb-2">{{ t( "zeit.title" ) }}</h3>
	<p class="text-body-2 mb-4">{{ t( "zeit.intro" ) }}</p>

	<v-row align="center" dense>
		<v-col cols="12" md="6">
			<div class="text-caption mb-1">{{ t( "zeit.labels.terrainFactor" ) }}</div>
			<v-slider v-model="terrainFactor"
				color="primary"
				:max="1.8"
				:min="0.8"
				step="0.05"
		...
		<v-col class="d-flex justify-end ga-2" cols="12" md="6">
			<v-btn color="primary" variant="flat" @click="addSegment">{{ t( "zeit.labels.addSegment" ) }}</v-btn>
			<v-btn variant="tonal" @click="resetSegments">{{ t( "zeit.labels.reset" ) }}</v-btn>
		</v-col>
	</v-row>
	...
	<v-table class="mt-2" density="compact">
		<thead>
			<tr>
				<th>#</th>
				<th>{{ t( "zeit.labels.distanceKm" ) }}</th>
				<th>{{ t( "zeit.labels.ascentM" ) }}</th>
				<th>{{ t( "zeit.labels.slope" ) }}</th>
				<th>{{ t( "zeit.labels.naismith" ) }}</th>
				<th>{{ t( "zeit.labels.tobler" ) }}</th>
				<th></th>
			</tr>
		</thead>
		<tbody>
	...
	<v-row class="mt-2" dense>
		<v-col cols="12" md="4">
			<v-sheet class="pa-3 rounded-lg" variant="outlined">
				<div class="text-caption text-medium-emphasis mb-1">{{ t( "zeit.labels.totals" ) }}</div>
				<div class="mono">{{ t( "zeit.labels.totalDistance" ) }}: {{ fmt( totalDistanceKm, 2 ) }} km</div>
				<div class="mono">{{ t( "zeit.labels.totalAscent" ) }}: {{ fmt( totalAscentM, 0 ) }} m</div>
				<div class="mono">{{ t( "zeit.labels.totalSegments" ) }}: {{ segments.length }}</div>
			</v-sheet>
		</v-col>
		<v-col cols="12" md="4">
			<v-sheet class="pa-3 rounded-lg" variant="outlined">
				<div class="text-caption text-medium-emphasis mb-1">{{ t( "zeit.labels.timeModel" ) }}</div>
				<div class="mono">{{ t( "zeit.labels.naismith" ) }}: {{ fmt( totalNaismithH, 2 ) }} h</div>
				<div class="mono">{{ t( "zeit.labels.tobler" ) }}: {{ fmt( totalToblerH, 2 ) }} h</div>
				<div class="mono">{{ t( "zeit.labels.difference" ) }}: {{ fmt( totalToblerH - totalNaismithH, 2 ) }} h</div>
			</v-sheet>
		</v-col>
		<v-col cols="12" md="4">
			<v-sheet class="pa-3 rounded-lg" variant="outlined">
				<div class="text-caption text-medium-emphasis mb-1">{{ t( "zeit.labels.recommendation" ) }}</div>
				<div class="mono">{{ t( "zeit.labels.planTime" ) }}: {{ fmt( planTimeH, 2 ) }} h</div>
				<div class="mono">{{ t( "zeit.labels.reserve" ) }}: {{ fmt( reserveH, 2 ) }} h</div>
				<div class="mono">{{ t( "zeit.labels.totalTarget" ) }}: {{ fmt( planWithReserveH, 2 ) }} h</div>
			</v-sheet>
		</v-col>
	</v-row>
...
const segments = ref( [
	{
		id: 1, distanceKm: 2.6, ascentM: 190
	},
	{
		id: 2, distanceKm: 1.9, ascentM: -60
	},
	{
		id: 3, distanceKm: 3.4, ascentM: 230
	}
] );
...
const status = computed( () => {
	if ( planWithReserveH.value < 2 ) {
		return { type: "success", message: t( "zeit.status.short" ) };
	}

	if ( planWithReserveH.value < 5 ) {
		return { type: "info", message: t( "zeit.status.medium" ) };
	}

	return { type: "warning", message: t( "zeit.status.long" ) };
} );

function addSegment() {
	segments.value.push( {
		id: nextId++, distanceKm: 1.8, ascentM: 120
	} );
}
