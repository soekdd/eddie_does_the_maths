...
<AppFrame>
	...
	<template #descriptionPart>
  		...
  		<h2 class="mb-4">IMO 1985 – Aufgabe 2: „Am Ende hat alles eine Farbe“</h2>
      ...
      <!-- Problem -->
      <v-card class="mt-2">
        <v-card-title class="text-h6">Aufgabenstellung</v-card-title>
        <v-card-text class="d-flex flex-column ga-3">
          ...
          <div class="mt-1">
            <strong>Zu zeigen:</strong> Alle Zahlen in <Katex :tex="'M'" /> müssen dieselbe Farbe haben.
          </div>
        </v-card-text>
      </v-card>
      ...
      <!-- Idee + Beweis -->
      <v-card class="mt-2">
        <v-card-title class="text-h6">Eddies Idee</v-card-title>
        <v-card-text class="d-flex flex-column ga-3">
          <div>
	...
	<template #interactivePart>
      <v-card class="mt-2">
        <v-card-title class="text-h6">Spielplatz: Prüfe es für konkrete n und k</v-card-title>
        <v-card-subtitle>
	...
	<template #calculationPart>
          ...
          <div class="d-flex flex-column ga-2">
            <div class="text-subtitle-1 font-weight-medium">Die „k-Sprung“-Kette (mod n)</div>
...
/**
 * Minimaler KaTeX-Renderer (katex ist "global bekannt").
 * Falls window.katex nicht existiert, wird einfach Plaintext angezeigt.
 */
const hasKatex = computed(() => typeof window !== "undefined" && !!window.katex);

function renderKatex(tex, displayMode = false) {
  try {
    if (!hasKatex.value) return tex;
