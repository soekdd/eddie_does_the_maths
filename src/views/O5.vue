	...
	<template #interactivePart>
		...
		<div class="eddie d-flex flex-column ga-3">
			...
			<div class="d-flex flex-wrap ga-3 align-center">
				...
				<v-btn @click="randomExample" variant="tonal">Zufall</v-btn>
			</div>
...
function solveLineCircle(B, u, radius = 1) {
	const bu = dot(B, u);
...
function orientedSecant(B, deg, radius = 1) {
	const dirs = [unitFromDeg(deg), mul(unitFromDeg(deg), -1)];

	for (const u of dirs) {
		const roots = solveLineCircle(B, u, radius);
...
function circleIntersections(c1, r1, c2, r2) {
	const d = dist(c1, c2);
	if (d < EPS) throw new Error("Kreise haben fast gleichen Mittelpunkt.");
	if (d > r1 + r2 + EPS) throw new Error("Kreise schneiden sich nicht.");
	if (d < Math.abs(r1 - r2) - EPS) throw new Error("Ein Kreis liegt im anderen ohne Schnitt.");

	const a = (r1 * r1 - r2 * r2 + d * d) / (2 * d);
	const h2 = r1 * r1 - a * a;
	if (h2 < -EPS) throw new Error("Numerischer Fehler bei Kreis-Schnitt.");
...
function lineIntersection(P1, P2, Q1, Q2) {
	...
	if (Math.abs(denom) < EPS) throw new Error("Geraden sind parallel oder fast parallel.");
...
function angleAt(A, B, C) {
	...
	if (n1 < EPS || n2 < EPS) throw new Error("Winkel mit zu kleinem Schenkel.");
...
function buildConfiguration({ bx, by, alpha, beta }) {
	const O = pt(0, 0);
	const B = pt(bx, by);
	if (dist(B, O) <= R + 0.02) {
	...
	const sec1 = orientedSecant(B, alpha, R);
	const sec2 = orientedSecant(B, beta, R);
	...
	if (area2 < 1e-4) throw new Error("A, B, C liegen nahezu auf einer Linie.");

	const cABC = circumcircle(A, B, C);
	const cKBN = circumcircle(K, B, N);
	const its = circleIntersections(cABC.center, cABC.radius, cKBN.center, cKBN.radius);

	const i1 = its[0];
	const i2 = its[1];
	const M = dist(i1, B) > dist(i2, B) ? i1 : i2;

	const X = lineIntersection(A, C, N, K);
	...
	eq.res1 = Math.max(
		Math.abs(eq.xmxb - eq.xkxn),
		Math.abs(eq.xmxb - eq.xo2on2)
	);
	eq.res2 = Math.max(
		Math.abs(eq.bmbx - eq.bnbc),
		Math.abs(eq.bmbx - eq.bo2on2)
	);

	const angleOMB = angleAt(O, M, B);
...
function runCheck() {
	...
	try {
		...
		result.value = buildConfiguration({ bx, by, alpha, beta });
	} catch (e) {
...
function randomExample() {
	...
	for (let attempt = 0; attempt < 200; attempt++) {
		...
		try {
			const out = buildConfiguration({ bx, by, alpha, beta });
