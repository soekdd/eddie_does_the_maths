...
function parseCoeffList(raw) {
  ...
  if (parts.length === 0) throw new Error("Koeffizienten: Bitte mindestens einen Wert eingeben.");
...
function coeffToLatex(c, power, isFirst) {
  const neg = c < 0n;
  const abs = neg ? -c : c;

  if (abs === 0n) {
    if (power === 0) return isFirst ? "0" : "+0";
    return isFirst ? "0" : "+0";
  }
...
function polynomialLatex(coeffs) {
  ...
  return coeffs
    .map((c, i) => coeffToLatex(c, degree - i, i === 0))
    .join(" ");
}
