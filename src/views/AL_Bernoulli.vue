        ...
        <v-card rounded="xl" class="pa-4">
          ...
          <v-window v-model="tab" class="mt-3">
            ...
            <v-window-item value="store">
              <v-row dense>
                <v-col v-for="section in storeLayout" :key="section.key" cols="12" md="4">
...
class Rat {
  ...
  div(r) {
    if (r.num === 0n) throw new Error("Division by zero (r=0).");
...
class CardVM {
  ...
  exec(card) {
    ...
    for (const d of dests) this.setV(d, r);

    this.step++;
    if (this.trace) {
      const dLabel = dests.map((d) => `V${d}`).join(",");
      this.logs.push(
        `#${String(this.step).padStart(2, "0")} ${card.label || ""} | ${card.op} → ${dLabel} = ${r.toString()}`
      );
    }
  }
...
function buildNoteGDeck() {
  ...
  // Setup (L1..L12)
  push(1, { op: "MUL", a: { V: 2 }, b: { V: 3 }, dest: [4, 5, 6], label: "2*n → V4,V5,V6" });
  push(2, { op: "SUB", a: { V: 4 }, b: { V: 1 }, dest: 4, label: "V4=2n-1" });
  push(3, { op: "ADD", a: { V: 5 }, b: { V: 1 }, dest: 5, label: "V5=2n+1" });
  push(4, { op: "DIV", a: { V: 4 }, b: { V: 5 }, dest: 11, label: "V11=(2n-1)/(2n+1)" });
  push(5, { op: "DIV", a: { V: 11 }, b: { V: 2 }, dest: 11, label: "V11/=2" });
  push(6, { op: "SUB", a: { I: 0 }, b: { V: 11 }, dest: 11, label: "A0=-V11" });
  push(7, { op: "ADD", a: { V: 13 }, b: { V: 11 }, dest: 13, label: "S+=A0" });
  push(8, { op: "DIV", a: { V: 6 }, b: { V: 2 }, dest: 11, label: "A1=(2n)/2" });
  push(9, { op: "ADD", a: { I: 0 }, b: { I: 1 }, dest: 17, label: "k=1" });
  push(10, { op: "MUL", a: { V: 11 }, b: { V: 21 }, dest: 12, label: "term=A1*B1" });
  push(11, { op: "ADD", a: { V: 13 }, b: { V: 12 }, dest: 13, label: "S+=term" });
  push(12, { op: "ADD", a: { V: 17 }, b: { I: 2 }, dest: 17, label: "k+=2" });
  ...
  for (const spec of loopSpecs) {
    push(13, { op: "SUB", a: { V: 6 }, b: { I: spec.f1 }, dest: 7, label: `V7=2n-${spec.f1}` });
    push(14, { op: "SUB", a: { V: 6 }, b: { I: spec.f2 }, dest: 8, label: `V8=2n-${spec.f2}` });
    push(15, { op: "MUL", a: { V: 11 }, b: { V: 7 }, dest: 11, label: "A*=V7" });
    push(16, { op: "MUL", a: { V: 11 }, b: { V: 8 }, dest: 11, label: "A*=V8" });
    push(17, { op: "ADD", a: { I: 0 }, b: { I: spec.d1 }, dest: 14, label: `V14=${spec.d1}` });
    push(18, { op: "ADD", a: { I: 0 }, b: { I: spec.d2 }, dest: 15, label: `V15=${spec.d2}` });
    push(19, { op: "MUL", a: { V: 14 }, b: { V: 15 }, dest: 16, label: "V16=V14*V15" });
    push(20, { op: "DIV", a: { V: 11 }, b: { V: 16 }, dest: 11, label: "A/=V16" });
    push(21, { op: "MUL", a: { V: 11 }, b: { V: spec.bCol }, dest: 12, label: `term=A*${spec.bLabel}` });
    push(22, { op: "ADD", a: { V: 13 }, b: { V: 12 }, dest: 13, label: "S+=term" });
    push(23, { op: "ADD", a: { V: 17 }, b: { I: 2 }, dest: 17, label: "k+=2" });
  }

  // Final output (L24)
  push(24, { op: "SUB", a: { I: 0 }, b: { V: 13 }, dest: 24, label: "B=-S" });
...
function parseFrac(s) {
  ...
  if (!t) throw new Error("Leerer Bruch.");
  if (/^-?\d+$/.test(t)) return new Rat(BigInt(t), 1n);
  const m = t.match(/^(-?\d+)\s*\/\s*(-?\d+)$/);
  if (!m) throw new Error(`Ungültiges Format: "${t}". Erlaubt: a/b oder ganze Zahl.`);
