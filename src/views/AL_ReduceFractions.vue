...
class IntVM {
  ...
  exec(card) {
    ...
    if (this.trace) {
      ...
      this.logs.push(
        `#${String(this.step).padStart(2, "0")} L${card.line} ${card.label || ""} | ${card.op} → ${dLabel} = ${r.toString()}`
      );
    }
  }
...
function buildReduceDeck(absNum, absDen) {
  ...
  push(1, { op: "ADD", a: { V: 5 }, b: { V: 1 }, dest: 11, label: "a:=|z|" });
  push(2, { op: "ADD", a: { V: 6 }, b: { V: 1 }, dest: 12, label: "b:=|n|" });
  push(3, { op: "ADD", a: { I: 0 }, b: { I: 0 }, dest: 16, label: "iter:=0" });
  ...
  while (b !== 0n) {
    guard++;
    if (guard > 512) throw new Error("Zu viele Schleifendurchläufe im Euklid-Loop.");

    const q = a / b;
    const t = q * b;
    const r = a - t;

    push(4, { op: "DIV", a: { V: 11 }, b: { V: 12 }, dest: 13, label: `[${guard}] q:=a/b` });
    push(5, { op: "MUL", a: { V: 13 }, b: { V: 12 }, dest: 14, label: `[${guard}] t:=q*b` });
    push(6, { op: "SUB", a: { V: 11 }, b: { V: 14 }, dest: 15, label: `[${guard}] r:=a-t` });
    push(7, { op: "ADD", a: { V: 12 }, b: { V: 1 }, dest: 11, label: `[${guard}] a:=b` });
    push(8, { op: "ADD", a: { V: 15 }, b: { V: 1 }, dest: 12, label: `[${guard}] b:=r` });
    push(9, { op: "ADD", a: { V: 16 }, b: { V: 2 }, dest: 16, label: `[${guard}] iter++` });

    a = b;
    b = r;
  }

  push(10, { op: "ADD", a: { V: 11 }, b: { V: 1 }, dest: 21, label: "g:=a" });
  push(11, { op: "DIV", a: { V: 3 }, b: { V: 21 }, dest: 22, label: "z':=z/g" });
  push(12, { op: "DIV", a: { V: 4 }, b: { V: 21 }, dest: 23, label: "n':=n/g" });
...
function toDecimalRounded(num, den, digits = 6) {
  const sign = num < 0n ? "-" : "";
  const n = bigAbs(num);
  const d = den;
...
async function run() {
  ...
  try {
    ...
    if (n === 0n) throw new Error("Nenner darf nicht 0 sein.");
    ...
    result.value = {
      ...
      reducedTex: String.raw`\frac{${z.toString()}}{${n.toString()}}=\frac{${zOut.toString()}}{${nOut.toString()}}`,
      decimal: toDecimalRounded(zOut, nOut, 6),
