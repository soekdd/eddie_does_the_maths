<template>
  ...
  >
    ...
    <!-- Background -->
    <rect class="bg" x="0" y="0" width="520" height="600" />
    ...
    <g v-else-if="layout">
      ...
      <!-- Axes -->
      <g class="axis">
        <line :x1="layout.left" :y1="layout.bottom" :x2="layout.xEnd + 16" :y2="layout.bottom" :marker-end="`url(#${arrowId})`" />
        <line :x1="layout.left" :y1="layout.bottom" :x2="layout.left" :y2="layout.yTop - 16" :marker-end="`url(#${arrowId})`" />
      </g>
      ...
      <g class="small">
        <text v-for="(v, i) in ticks.x" :key="`xl-${i}`" :x="xPix(v)" :y="layout.bottom + 28" text-anchor="middle">
          {{ v }}
        </text>
        <text v-for="(v, i) in ticks.y" :key="`yl-${i}`" :x="layout.left - 18" :y="yPix(v) + 4" text-anchor="end">
          {{ v }}
        </text>
      </g>
      ...
      <text v-if="yLabel.length<2" class="label" :x="layout.left - 55" :y="layout.yTop + 8">
        {{ yLabel }}
      </text>
      <text v-else class="label" :x="layout.left - 152" :y="layout.yTop + 8" :transform="`rotate(-90 ${layout.left - 52} ${layout.yTop + 8})`">
        {{ yLabel }}
      </text>
      ...
      <g v-if="constraintLines">
        ...
        <text v-if="constraintLines.x" class="small" :x="constraintLines.x.line.x1 + 4" :y="layout.yTop + 16">
          {{ constraintLines.x.label }}
        </text>
        <text v-if="constraintLines.y" class="small" :x="layout.left + 6" :y="constraintLines.y.line.y1 - 6">
      ...
      <g v-for="(p, i) in drawPoints" :key="`pc-${i}`">
        <circle class="point" :cx="xPix(p.x)" :cy="yPix(p.y)" r="7" />
        <circle class="pointFill" :cx="xPix(p.x)" :cy="yPix(p.y)" r="2.5" />
      </g>
      ...
      <g v-if="vector">
        ...
        <text class="small" :x="vector.mx" :y="vector.my" :transform="`rotate(${vector.ang} ${vector.mx} ${vector.my})`">
          {{ vector.label }}
        </text>
      </g>
      ...
      <g v-if="showLegend && legend" class="small">
        <text v-for="(ln, i) in legend.lines" :key="`l-${i}`" :x="legend.x" :y="legend.y0 + i * 20">
          {{ ln }}
        </text>
        <text
...
function gridStepFor(maxUnits) {
  if (maxUnits <= 30) return 1
  if (maxUnits <= 60) return 2
  if (maxUnits <= 120) return 5
  if (maxUnits <= 240) return 10
  return 20
}

function majorStepFor(maxUnits) {
  if (maxUnits <= 12) return 2
  if (maxUnits <= 25) return 5
  if (maxUnits <= 50) return 10
  if (maxUnits <= 100) return 20
  return 50
}
...
const model = computed(() => {
  ...
  const g = gcd(a, b)
  if (c % g !== 0) {
    return { ok: true, a, b, c, points: [], g, dx: b / g, dy: a / g, message: 'Keine ganzzahligen Lösungen.' }
  }
...
const ticks = computed(() => {
  if (!layout.value || !model.value.ok) return { x: [], y: [] }
  const l = layout.value

  const x = []
  for (let v = 0; v <= l.xMaxUnits + 1e-9; v += l.majorX) x.push(v)
  if (x.at(-1) !== l.xMaxUnits) x.push(l.xMaxUnits)

  const y = []
  for (let v = 0; v <= l.yMaxUnits + 1e-9; v += l.majorY) y.push(v)
  if (y.at(-1) !== l.yMaxUnits) y.push(l.yMaxUnits)

  return { x, y }
})

const lineSegment = computed(() => {
  if (!layout.value || !model.value.ok) return null
  const l = layout.value
  const { a, b, c } = model.value
  ...
  const uniq = []
  for (const p of pts) {
    if (uniq.some((q) => Math.abs(q.x - p.x) < 1e-6 && Math.abs(q.y - p.y) < 1e-6)) continue
    uniq.push(p)
  }
  ...
  return {
    d: `M ${xPix(p0.x)} ${yPix(p0.y)} L ${xPix(p1.x)} ${yPix(p1.y)}`,
  }
})
...
const constraintLines = computed(() => {
  ...
  if (x > 0 && x <= l.xMaxUnits) {
    out.x = {
      line: { x1: xPix(x), y1: l.yTop, x2: xPix(x), y2: l.bottom },
      label: `${props.xVar} ≥ ${x}`,
    }
  }
  ...
  if (y > 0 && y <= l.yMaxUnits) {
    out.y = {
      line: { x1: l.left, y1: yPix(y), x2: l.xEnd, y2: yPix(y) },
      label: `${props.yVar} ≥ ${y}`,
    }
  }
...
const legend = computed(() => {
  ...
  const extra = []
  if (model.value.message) extra.push(model.value.message)
  if (hasHiddenLabels.value) extra.push(`(… und ${drawPoints.value.length - labelPoints.value.length} weitere)`)
